...
array = '/scratch/imaging/projects/bh_recon_eval/arrays/EHT2019.txt'
...
nsamp = 10000 
def Prepare_EHT_Data(fov_param, flux_label, blur_param, sefd_param=1, eht_array='eht2019', target='m87', data_augmentation=False, npix=32):
    """
    ...
    if eht_array == 'eht2019':
        ...
        array = '/scratch/imaging/projects/bh_recon_eval/arrays/EHT2025.txt'
    ...
    #obs2 = obs.copy()
    #obs2.data['vis'] = obs_data['vis']
    
    # xdata is blurred by 0.3*fwhm
    # xdata_blur is blurred by an additional 0.3*fwhm
...
##############################################################################
# Define custom callback to compute loss on all 3 datasets
##############################################################################
class AdditionalValidationSets(tf.keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        for validation_set in self.validation_sets:
            ...
            #print(validation_set_name, "_mae: ", mae, ' ', validation_set_name, "_chisq: ", chisq.eval())
            #sess.close()
            
            #tf.print(valuename, ": ", chisq, output_stream=sys.stderr)
...
def Train_VisNet(eht_array, target, fov_param, flux_label, blur_param, sefd_param, lr, nb_epochs_train, batch_size = 32, n_ising_layers = 5, models_dir='', savefile_name='nn_params', data_augmentation=False, weather=False):
    ...
    model.compile(optimizer=opt, loss=['mae', chisq_loss], loss_weights=[1, 0])
    ...
    model.load_weights('Models/thnoise_models/thnoise_model_nochisqbest.hdf5')
    #model.load_weights('thnoise_models/unet_vis_sefd1_blur0.3_lr0.01_batch256chisq1500epochsbest.hdf5')
    #model.load_weights('nonoise_models/nonoise_model1best.hdf5')
    init_epoch= 0 

    # Define additional validation sets
    ...
    # Define validation set with MNIST digits and fashion
    ...
    #valid_vis = np.load('mnist_vis_nonoise.npy')
    ...
    #valid_vis = np.concatenate([np.array(valid_vis).real, np.array(valid_vis).imag], axis=-1)
    
    #xdata_vis = np.load('fashion_augmented_vis_nonoise.npy')
    ...
    #xdata_vis = np.concatenate([np.array(xdata_vis).real, np.array(xdata_vis).imag], axis=-1)
    
    
    #obs2 = obs.copy()
    #bs2.data['vis'] = xdata_vis[0] #+ np.random.normal(sigma[:, 1])
    #obs2.data['sigma'] = sigma[:,1]
    #obs2.save_uvfits('obs_nonoise.uvfits')
    
    history = AdditionalValidationSets([(fashion_xdata, fashion_vis, fashion_blur, 'fashion_data'),
...
if __name__ == '__main__':    
    ...
    #savefile_name = 'unet_vis_sefd'+str(sefd_param)+'_blur'+str(blur_param)+'_lr'+str(lr)+'_batch'+str(batch_size)+'chisq1e-2_'+str(nb_epochs_train)+'epochs'
    savefile_name = 'thnoise_model_nochisq'
    #if data_augmentation:
    #    savefile_name = 'unet_vis_sefd'+str(sefd_param)+'_blur'+str(blur_param)+'_lr'+str(lr)+'_batch'+str(batch_size)+"_dataaugmentation"
    models_dir = 'Models/thnoise_models/'
